        ...
        for (const [index, item] of sewa.entries()) {
            if (Date.now() >= item.expired) {
                ...
                if (groupIds.includes(item.groupId)) {
                    const teks = `Waktu sewa di grup ini sudah habis, bot akan keluar otomatis, silahkan chat pemilik bot untuk sewa ulang`;
                    await
conn.sendContact(item.groupId.owner.map( i => i.split("@")[0]), nomer)                    
