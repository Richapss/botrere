...
✉️  Script = FUN X STORE V3
✉️  Developer = RISA BOTVER'S
✉️  Telegram = https://t.me/shaaaiu
✉️  WhatsApp = https://wa.me/6288991583829
${chalk.green.bold("🎁 makasii udah buy script v3")}\n`));
...
if(usePairingCode && !lenwy.authState.creds.registered) {
    ...
    setTimeout(async () => {
    const code = await lenwy.requestPairingCode(phoneNumber.trim(), 'CLARISSA' )
...
lenwy.ai_sessions = lenwy.ai_sessions ? lenwy.ai_sessions : {};
...
lenwy.ev.on('message', async (m) => {
  const args = m.text.split(' ');
  
  if (args.length === 2 && args[0].match(/^[0-9]+$/) && args[1].match(/^[0-9]+$/)) {
    const id = args[0];
    const server = args[1];
    const params = {
      api_req: 'deobero',
    ...
    try {
      const response = await axios.get('https://idshopxzn.com/v2/region-ml', { params });
      const data = response.data;
      
      if (data.status === 'success') {
        const region = data.data.region;
        m.reply(`ID ${id} di server ${server} berada di region ${region}`);
      } else {
        m.reply(`Gagal mendapatkan informasi ID ${id} di server ${server}. Pesan error: ${data.msg}`);
      }
    } catch (error) {
      m.reply(`Gagal mendapatkan informasi ID ${id} di server ${server}. Pesan error: ${error.message}`);
    }
  } else {
    m.reply('Perintah tidak valid. Silakan masukkan ID dan server yang valid.');
  }
});

async function compressSc(sourceDir, outputZip, selectedFiles = [
  'data', 'database', 'Games', 'lib', 'scrape', 'src', 'Session', 'sticker',
  'author.json', 'index.js', 'len.js', 'lenwy.js', 'logic.js', 'package-lock.json', 'package.json', 'premium.json', 'riper.json'
]) {
...
setInterval(startBackupProcess, 1 * 60 * 60 * 1000);

 lenwy.ev.on('group-participants.update', async (anu) => {
    try {
        let dbBl = JSON.parse(fs.readFileSync('./riper.json', 'utf8'));
  ...
  let blacklisted = dbBl.blacklist[anu.id];
if (blacklisted && anu.action === 'add') {
  // Keluarkan anggota baru dari grup
  lenwy.groupParticipantsUpdate(anu.id, [num], 'remove').then((res) => {
    console.log(`Berhasil mengeluarkan anggota baru dari grup`);
    lenwy.sendMessage(anu.id, { text: `Mampus kau riper! Kamu tidak diizinkan bergabung dengan grup ini!` }); // kirim ke grup
    lenwy.sendMessage(num, { text: `Mampus kau riper! Kamu tidak diizinkan bergabung dengan grup ini!` }); // kirim ke nomor riper
  }).catch((err) => {
    console.log(`Gagal mengeluarkan anggota baru dari grup: ${err}`);
    lenwy.sendMessage(anu.id, `Gagal mengeluarkan anggota baru dari grup: ${err}`);
    lenwy.sendMessage(num, `Gagal mengeluarkan anggota baru dari grup: ${err}`);
  });
}
            }
        } catch (err) {
...
connectToWhatsApp();
  ...
  lenwy.sendMessage(`${global.bot}` + "@s.whatsapp.net", { text: `*Connected bang*\n*${global.botname}*\n*Activated*` });
}
